...
Flagship uses the matrix as-is. Utility applies exactly these seven changes and no others:
...
6. `privacy-reviewer` → `off(merged-into: security-reviewer — privacy MODE)`. One role, two
...
7. `chief-of-staff`, `test-automation-engineer`, `reliability-engineer`, `red-team-agent` →
