...
# S.2 lives on, in a different shape. This file no longer parses flags at all, so the `shift 2`
# hang and the "a token in a value position is a value" rule are not re-implemented here — they
# cannot be reached. Arguments go to node as an argv array; `messages.mjs` rejects a value-less flag
# and an unknown flag, and nothing is ever interpolated into a shell string. S.4 redaction moved
# with the write: `messages.mjs` scrubs summary, body, decision and evidence through lib/redact.mjs
